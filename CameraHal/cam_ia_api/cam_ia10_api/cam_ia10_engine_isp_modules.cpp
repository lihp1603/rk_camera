...
)
{
	...
	if (enable_mode == HAL_ISP_ACTIVE_FALSE) {
		...
		cam_ia10_isp_flt_config(HAL_ISP_ACTIVE_SETTING, &flt_default_cfg, flt_result);
	} else {
		ALOGE("%s:error enable mode %d!",__func__,enable_mode);
		result = RET_FAILURE;
	}
...
)
{
	...
	if (enable_mode == HAL_ISP_ACTIVE_FALSE) {
		...
		CamCalibGocGlobal_t *pGocGlobal;
		result = CamCalibDbGetGocGlobal(hCamCalibDb, &pGocGlobal);
		if (result != RET_SUCCESS) {
			ALOGD("fail to get pGocGlobal, ret: %d", result);
		}
		...
		if (pGocGlobal != NULL) {
			goc_enable_mode = pGocGlobal->enable_mode < 0 ? 
				HAL_ISP_ACTIVE_SETTING : HAL_ISP_ACTIVE_MODE(pGocGlobal->enable_mode);
			goc_def_cfg.mode = pGocGlobal->def_cfg_mode < 0 ? 
				HAL_ISP_GAMMA_SEG_MODE_LOGARITHMIC : HAL_ISP_GAMMA_SEG_MODE_e(pGocGlobal->def_cfg_mode);
		}

		if (pGocGlobal != NULL && pGocGlobal->GammaY[CAMERIC_ISP_GAMMA_CURVE_SIZE - 2] > 0) {
			for (int index = 0; index < (CAMERIC_ISP_GAMMA_CURVE_SIZE); index++)
				goc_def_cfg.gamma_y[index] = pGocGlobal->GammaY[index];
...
)
{
    ...
    MaxNumOfPixel = MaxNumOfPixel * \
			( ((uint32_t)height / 5 )* ((uint32_t)width / 5 ) );

    switch ( mode )
    {
        case CAMERIC_ISP_HIST_MODE_RGB_COMBINED:
            {
                square = (uint8_t)( (3 * MaxNumOfPixel) / 0x000FFFFF );
                break;
            }
        ...
        case CAMERIC_ISP_HIST_MODE_Y:
            {
                square = (uint8_t)( MaxNumOfPixel / 0x000FFFFF );
                break;
            }
...
)
{
	...
	if (enable_mode == HAL_ISP_ACTIVE_FALSE) {
		...
		if (result != RET_SUCCESS) {
			ALOGD("fail to get pWdrGlobal, ret: %d", result);
		}
