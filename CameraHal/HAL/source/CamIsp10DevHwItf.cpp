...
bool CamIsp10DevHwItf::Path::releaseBufToOwener(weak_ptr<CameraBuffer> camBuf)
{
    int ret = true;
...
bool CamIsp10DevHwItf::initHw(int inputId)
{
	...
	if (!m_flag_init) {
		...
		mISPDev = shared_ptr<CamIsp10CtrItf>(new CamIsp10CtrItf());
...
int CamIsp10DevHwItf::configIsp_l(struct isp_supplemental_sensor_mode_data *sensor)
{
...
int CamIsp10DevHwItf::setExposure(unsigned int exposure, unsigned int gain, unsigned int gain_percent)
{
	int ret;
...
int CamIsp10DevHwItf::setFPSCtrl()
{
	int ret = 0;
...
/*
bool CamIsp10DevHwItf::enumSensorFmts(vector<frm_info_t> &frmInfos)
{
...
int CamIsp10DevHwItf::setWhiteBalance(HAL_WB_MODE wbMode)
{
...
int CamIsp10DevHwItf::setAntiBandMode(enum HAL_AE_FLK_MODE flkMode)
{
	if(mFlkMode == flkMode)
		return 0;
	mFlkMode = flkMode;
...
int CamIsp10DevHwItf::setAeBias(int aeBias)
{
