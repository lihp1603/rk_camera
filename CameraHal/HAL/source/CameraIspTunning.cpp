...
#define TUNNING_MAX_GAIN (8.0f)
#define TUNNING_MAX_INTEGRATION_TIME (0.047f)
...
bool CameraIspTunning::processFrame(shared_ptr<CameraBuffer> inBuf,shared_ptr<CameraBuffer> outBuf)
{
	...
	if (metaData){
			...
			if ((curtime < 0.0001f) || (curgain < 1.0f)) {
	...
	if(curTuneTask->mExpose.exposuseMode == EXPOSUSE_MODE_MANUAL){
		...
		if((fabs(curgain - mCurGain) > 0.0001) || (fabs((curtime - mCurIntegrationTime)) > 0.0001)){
		...
		if(curTuneTask->mTuneFmt == CAMERIC_MI_DATAMODE_RAW12 ){
			...
			if((curTuneTask->mExpose.integrationTimeStep != 0)|| (curTuneTask->mExpose.gainStep != 0)){
				...
				int filter_result = ispTuneDesiredExp((int)(inBuf->getVirtAddr()),
	...
	snprintf( szBaseFileName, sizeof(szBaseFileName)-1, "%st%0.3f_g%0.3f", 
		RK_ISP_TUNNING_IMAGE_STORE_PATH,curtime, curgain );
	szBaseFileName[99] = '\0';
	...
	ispTuneStoreBuffer(curTuneTask, inBuf.get(),szBaseFileName, 0);
	PROCESS_OVER:
    ...
    if ( 1)
    {
        ...
        fprintf( pFile,
                ...
                putHeader ? "" : "\n",pBuffer->getWidth(), pBuffer->getHeight(),
                        0, 0, TimeStampUs, is16bit ? 65535 : 255 );
