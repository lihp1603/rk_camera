...
bool StreamPUBase::releaseBufToOwener(weak_ptr<CameraBuffer> camBuf)
{
	//ALOGD("%s:%d ",__func__,__LINE__);
	bool ret = false;
	...
	if (spBuf.get() != NULL)
	{
		for (list<shared_ptr<CameraBuffer> >::iterator i = mOutBuffers.begin(); i != mOutBuffers.end(); i++) 
		{
...
bool StreamPUBase::bufferReady(weak_ptr<CameraBuffer> buffer, int status)
{
	shared_ptr<CameraBuffer> spBuf = buffer.lock();
...
bool StreamPUBase::start()
{
	...
	if (mNeedStrmTh) {
		...
		if (mStrmPUThread->run() == RET_SUCCESS)
			ret = true;
	} else {
		ret = true;
	}
...
bool StreamPUBase::processFrame(shared_ptr<CameraBuffer> inBuf,shared_ptr<CameraBuffer> outBuf)
{
	UNUSED_PARAM(inBuf);
	UNUSED_PARAM(outBuf);
	ALOGD("%s:%d ",__func__,__LINE__);
...
bool StreamPUBase::streamThread::threadLoop(void)
{
	...
	if (PUCALL(mState) == STREAMING)
	{
		...
		shared_ptr<CameraBuffer> camBuf;	
		...
		if (!PUCALL(mInBuffers.empty()))
		{
			...
			if (!PUCALL(mShareInBuf)) {
				if (PUCALL(mBufferPool.empty())) {
					...
					//discard this frame
					camBuf->decUsedCnt();
					ALOGW("%s: no available PU buffer now",__func__);
					goto exit;
				}
			...
			if (err == false) {
				...
				for (list<NewCameraBufferReadyNotifier* >::iterator i = PUCALL(mBufferReadyNotifierList.begin());
					...
					ret = (*i)->bufferReady(weak_ptr<CameraBuffer>(outBuf) , 0);
