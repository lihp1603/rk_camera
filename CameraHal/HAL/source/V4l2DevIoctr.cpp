...
int V4L2DevIoctr::queueBuffer(shared_ptr<CameraBuffer> &buffer)
{
    struct v4l2_buffer v4l2Buf;
    int ret;

    //ALOGV("%s: %s index %d, addr 0x%p, capacity %d", __func__, string(), buffer.getIndex(), buffer.getVirtAddr(), buffer.getCapacity());
...
int V4L2DevIoctr::dequeueBuffer(shared_ptr<CameraBuffer> &buffer, unsigned long timeout_ms)
{
    struct v4l2_buffer v4l2Buf;
    int ret;
   ...
   // ALOGV("%s: %s timeout %ldms", __func__, string(), timeout_ms);
    ...
    {
		osMutexLock(&mBufferQueueLock);
        for (list<weak_ptr<CameraBuffer> >::iterator i = mBufferQueue.begin(); i != mBufferQueue.end(); i++) {
			shared_ptr<CameraBuffer> tmpSpBuf;
			if (!(*i).expired())
				tmpSpBuf = (*i).lock();
...
int V4L2DevIoctr::setFormat(unsigned int &v4l2PixFmt, unsigned int &width, unsigned int &height, unsigned int stride) const
{
...
bool V4L2DevIoctr::memMap(shared_ptr<CameraBuffer> &buffer)
{
...
bool V4L2DevIoctr::memUnmap(shared_ptr<CameraBuffer> &buffer)
{
	if(V4L2_MEMORY_MMAP == mV4L2MemType)
		{
...
bool V4L2ISPDevIoctr::memMap(shared_ptr<CameraBuffer> &buffer)
{
	char* virtAddr = NULL;
	if (mMaped == 0) {
		virtAddr = (char*)mmap(0 /* start anywhere */ ,
...
bool V4L2ISPDevIoctr::memUnmap(shared_ptr<CameraBuffer> &buffer)
{
	if (mMaped <= 0 )
		return true;
	if (--mMaped == 0) {
