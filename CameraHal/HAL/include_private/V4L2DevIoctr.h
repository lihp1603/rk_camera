...
class V4L2DevIoctr
{
	public:
		...
		int queueBuffer(shared_ptr<CameraBuffer> &buffer);
		int dequeueBuffer(shared_ptr<CameraBuffer> &buffer, unsigned long timeout_ms);
		int setFormat(uint32_t &v4l2PixFmt, uint32_t &width, uint32_t &height, uint32_t stride) const;
		...
		virtual bool memMap(shared_ptr<CameraBuffer> &buffer);
		virtual bool memUnmap(shared_ptr<CameraBuffer> &buffer);
	...
	private:
		...
		list<weak_ptr<CameraBuffer> > mBufferQueue;
		bool mError;
		bool mStreaming;
...
class V4L2ISPDevIoctr :public V4L2DevIoctr{
	public:
		...
		virtual bool getBufferMetaData(int index,struct HAL_Buffer_MetaData* metaData);
		/* used for meta data*/
		bool memMap(shared_ptr<CameraBuffer> &buffer);
		bool memUnmap(shared_ptr<CameraBuffer> &buffer);
	private:
