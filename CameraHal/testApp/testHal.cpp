...
{
public:
	virtual bool releaseBufToOwener(weak_ptr<CameraBuffer> camBuf) { UNUSED_PARAM(camBuf);return false;}
};
...
public:
	...
	virtual bool bufferReady(weak_ptr<CameraBuffer> buffer, int status)
	{
		...
		if (spCamBuf.get())
		{
			...
			//ALOGD("got a new frame,frame index %d,virAddr %p",
			//	spCamBuf->getIndex(),spCamBuf->getVirtAddr());
			//send to display
			#ifdef USE_RK_DISPLAY
			rk_fb_config(&gFb,
						0,	/*win id*/
...
void testSofiaIsp(struct sofiaTestPara_s* para)
{
	...
	ALOGD("construct ISP dev........");
        //shared_ptr<CamHwItf> testIspDev =  shared_ptr<CamHwItf>(new CamIsp11DevHwItf());//getCamHwItf();
			...
			if (dmaPath->prepare(frmFmt,bufPool,4,0)) {
				...
				NewCameraBufferReadyNotifier * mMpBufNotifer = new CambufNotifierImp();
                                if (mpath->prepare(frmFmt,4,*(bufAlloc.get()),false,0)) {
                                        ...
                                        NewCameraBufferReadyNotifier * mBufNotifer= new CambufNotifierImp();
                                        //set buffer notifer required fmt
...
void testCifDev(int videoDevNum) {
		//new ISP DEV
		ALOGD("construct CIF dev........");
		shared_ptr<CamHwItf> testCifDev =  shared_ptr<CamHwItf>(new CamCifDevHwItf());//getCamHwItf();
		ALOGD("init CIF dev......");
		if (testCifDev->initHw(videoDevNum) == false)
